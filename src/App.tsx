...
const pickOr = <T,>(arr: T[], fallback: T) => (arr.length ? pick(arr) : fallback);
...
function generateCampaign(): Campaign {
  ...
  const characters: Character[] = Array.from({ length: 18 }).map((_, i) => {
    ...
    const home = pickOr(settlements.filter((s) => s.ownerFactionId === factionId), settlements[0]);
    return {
      id: `c${i + 1}`,
  ...
  characters.forEach((c) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (target) c.relationships[target.id] = r(-40, 80);
    }
  });
      ...
      <div className="grid">
        ...
        <section>
          ...
          <div className="actions">
            ...
            <select key={`travel-${campaign.month}`} onChange={(e) => e.target.value && doAction('travel', e.target.value)} defaultValue="">
              <option value="">Travel…</option>
              {campaign.settlements.filter((s) => s.id !== location.id).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            <select key={`attack-${campaign.month}`} onChange={(e) => e.target.value && doAction('attack', e.target.value)} defaultValue="">
